	...
	'Score': {
		...
		'Show':function($f,$tab,$id){
			...
			if($f==1){
				...
				str += '<div class="star"><div class="woyao">打分：</div><ul></li><li class="star_current"></li>';
				for(var i=1;i<=10;i++){ str += '<li><a value="'+i+'" class="star_'+i+'">'+i+'</a></li><li id="star_tip"></li>'; }
				str += '</ul>';
				str +='<em id="star_ge">.0</em><em id="star_shi">0</em></div>';
				//提示等级：<span id="star_tip"></span>
				//提示分数：<span id="star_hover"></span>
				//提示前面分数：<span id="star_shi">0</span>
				//提示后面分数：<span id="star_ge">.0</span>
			...
			$.ajax({type: 'get',url: SitePath + this.ajaxurl + '&tab='+$tab+'&id='+$id,timeout: 5000,
				...
				success: function($r){
					...
					if($f==1){
						...
						var tip=new Array("","太差","很差","讨厌","可以","一般","凑合","很好","喜欢","推荐","很棒");
