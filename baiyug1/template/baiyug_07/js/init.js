        ...
        bannerBtnLi.eq(0).addClass(options.figureBtnAct).siblings().removeClass(options.figureBtnAct);
         ...
         function shrink(){
          ...
          if (winWidth <= 1280) {
              ...
              bannerBtn.width(1240);
              bannerSwapBtn.width(1240);
...
function resizeWindow(){
  ...
  //当小于等于1200px的时候
   if (winWidth <= 1240) {
     ...
     $('.zt-top-inner,.mhm-menu,.mhs-inner,.container-inner,.zt-footer-inner,.details-head-inner,.details-body').width(1240);
     $('.mhm-inner,.mhm-share,#banner,.zt-footer,.footer_inner,.movie-top,.clear,.channel-flash,.site_focus,.focus_inner,.focus_img,.slider').css('width','100%');
    }
  }
  resizeWindowTwo();
...
if ($('.movie-header-search').get(0)) {
   ...
   function suspend(){
     ...
     if ( $(window).scrollTop() > 40 ) {
         ...
         headerNav.show();
         searchTxt.css('margin-left','200px');
